'use strict'
// console.dir(window);
// console.dir(document);

// ========== Dom navigation ===================
// const list = document.querySelector('.list');

// let fragment = document.createDocumentFragment();

// const newElement = document.createElement('li');
// newElement.textContent = "Ex";

// const newElement1 = document.createElement('li');
// newElement1.textContent = "Ex1";

// const newElement0 = document.createElement('li');
// newElement0.textContent = "Ex0";
// newElement0.dataset.asd = "qwerty";

// const newElementClone = newElement.cloneNode(true);
// newElementClone.textContent = "fghnfgxdfsdd";

// fragment.appendChild(newElement);
// fragment.appendChild(newElementClone);
// // fragment.appendChild(newElement1);
// fragment.prepend(newElement0);
// fragment.append(newElement1);

// list.appendChild(fragment);

// const findElem = document.querySelector('li[data-asd=qwerty]');
// console.log('findElem :', findElem);
// list.removeChild(findElem);




// const newElement = document.createElement('li');
// const newSpan = document.createElement('span');
// newSpan.textContent = "Example";
// newElement.appendChild(newSpan);
// list.appendChild(newElement);











// console.log(list);
// ===   querySelector    ===
// console.dir(list.childNodes);
// console.dir(list.children);
// console.dir(list.lastChild);
// console.dir(list.lastElementChild);

// ===   querySelectorAll    ===
// const arrLi = list.querySelectorAll('li')
// arrLi[0].dataset.number = "vghfgfhdfhdgdgfgn";
// console.dir(arrLi);
// arrLi[0].style.backgroundColor = 'teal';
// arrLi[0].classList.add("listNewClass")




// ===   property  &  attributes    ===
// const span = document.querySelector('.title111');
// console.log(span.textContent);
// span.textContent = "123456";
// console.log(span.textContent);


// const image = document.querySelector('.image');
// // console.dir(image);
// const src = "data:image/jpeg;base64,";
// // image.src = "data:image/jpeg;base64,"
// image.setAttribute("src", src);
// console.log(image.attributes)

// ===       ===


const list = document.querySelector('.list');

// const innerText = `
// <li><span>Text</span></li>
// <li><a href="google.com">Google</a></li>
// <li>fghfhgfgsfsdfsdfhghfh</li>
// <li>fghfhgfgsdffdsfdshghfh</li>
// `
// list.innerHTML = innerText;

// const newInnerText = `
// <li>fa</li>
// <li>gdfgfd</li>
// `;

let fragment = document.createDocumentFragment();
fragment.appendChild
const array = [{ name: "Boris", age: 25, src: "https://gamepedia.cursecdn.com/camelotunchained_gamepedia/thumb/5/53/Pict_spotlight.png/250px-Pict_spotlight.png?version=ee15fdf92b69dd942fc9175ffbd03fae" }, { name: "gfd", age: 34, src: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQzPOiz05fE0gXARNtbD53DpRlFgRCHKd4QMhnsTURFC-C2NcN2" }];
const getData = function (array) {

    const data = array.reduce((acc, { name, age, src }) => {
        acc.appendChild(document.createElement("li"));
        return acc;
    }, fragment)

    console.log('data :', data);
    return data;
}

list.insertAdjacentHTML("afterbegin", getData(array));











